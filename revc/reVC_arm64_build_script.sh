...
## This script is supposed to be run from inside a Vice City PC folder
...
# To build reVC you need to switch branches:
git checkout miami
...
# Build for Arm:
make config=release_macosx-arm64-librw_gl3_glfw-oal reVC

# The binary will be in bin/macosx-arm64-librw_gl3_glfw-oal/release
cd ../bin/macosx-arm64-librw_gl3_glfw-oal/release
# It will likely be created with .app extension, but this needs to be removed
mv reVC.app reVC
mv reVC ../../../gamefiles
cd ../../../../
mv re3/gamefiles reVC\ gamefiles
rm -r -f re3

# Copy the newly built reVC gamefiles into the appropriate folder, and overwrite if necessary
cp reVC\ gamefiles/reVC reVC
mv reVC\ gamefiles/neo neo
rsync -r ./reVC\ gamefiles/data/ data
rsync -r ./reVC\ gamefiles/models/ models
rsync -r ./reVC\ gamefiles/TEXT/ TEXT
...
# Create PkgInfo
PKGINFO="-n APPLreVC"
echo "${PKGINFO}" > "Vice City.app/Contents/PkgInfo"
...
cp ${GAME_ID} ${GAME_ID}.ini ${GAME_TITLE}.app/Contents/MacOS/

# Get icon from macosicons.com
curl -o ${GAME_TITLE}.app/Contents/Resources/${GAME_ID}.icns https://parsefiles.back4app.com/JPaQcFfEEQ1ePBxbf6wvzkPMEqKYHhPYv8boI1Rc/687fe68a01ffc1c7bcb2d4b6043fbbc7_NiEJ6CNA0j.icns
